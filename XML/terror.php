...
$query = $conexion->query("SELECT * FROM mis_productos WHERE Genero LIKE 'Terror'");		
$xml = new XMLWriter();
$xml->openMemory();
$xml->setIndent(true);
$xml->setIndentString('	'); 
...
$xml->startElement("Tienda"); //elemento TIENDA
         ...
         $xml->startElement("Terror"); //elemento responsable
...
header("Content-Disposition: attachment;filename=Terror.xml");
header('Expires: 0');
header('Pragma: cache');
