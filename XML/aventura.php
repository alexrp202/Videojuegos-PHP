...
$query = $conexion->query("SELECT * FROM mis_productos WHERE Genero LIKE 'Aventura'");		
$xml = new XMLWriter();
$xml->openMemory();
$xml->setIndent(true);
$xml->setIndentString('	'); 
...
$xml->startElement("Tienda"); //elemento TIENDA
         ...
         $xml->startElement("Aventura"); //elemento responsable
...
header("Content-Disposition: attachment;filename=Aventura.xml");
header('Expires: 0');
header('Pragma: cache');
