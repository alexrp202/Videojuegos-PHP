...
$query = $conexion->query("SELECT * FROM mis_productos WHERE Genero LIKE 'Puzzle'");		
$xml = new XMLWriter();
$xml->openMemory();
$xml->setIndent(true);
$xml->setIndentString('	'); 
...
$xml->startElement("Tienda"); //elemento TIENDA
         ...
         $xml->startElement("Puzzle"); //elemento responsable
...
header("Content-Disposition: attachment;filename=Puzzle.xml");
header('Expires: 0');
header('Pragma: cache');
